...
define([
    ...
    function Export(type, filename, fset) {
        
        this._link = $('#download-link');
        this._filename = filename;
        
        if (!this._link.attr('download') || !fset || fset.length === 0) {
            return false;
        }
        this._link.removeAttr('href');
    ...
    Export.prototype = {
        ...
        gpx : function (fset) {
            var string = new ol.format.GPX().writeFeatures(fset),
                base64 = this.strToBase64('<?xml version="1.0" encoding="utf-8"?>' + string);
            this._link.attr('download', this._filename);
            this._link.attr('href', 'data:text/gpx+xml;base64,' + base64);
        },
        
        forceDownload : function () {
            var clickEvent = document.createEvent("MouseEvent");
            clickEvent.initMouseEvent("click", true, true, window, 0, 0, 0, 0, 0, false, false, false, false, 0, null);
            document.getElementById("download-link").dispatchEvent(clickEvent);
            return false;
        },
